...
CONFIG = {
    ...
    # 提示词模板
    "prompts": {
        "system_prompt": """你是一个专业的工业自动化设备配置专家。你的任务是帮助用户设计和生成设备配置。
...
""",
        ...
        "ai_recommend_template": """根据用户的需求，生成一个设备配置列表。请注意，设备配置列表应包含每个设备的输入信号和输出信号，并且每个设备的输入信号应与其功能相关联，输出信号应表示设备执行的动作。
...
请总结以上信息，为我生成一个如下的需求表，包含项目的大致简述、每个设备的名称和功能。
#### 以下是需求表示例 ###
**输出格式要求：**
...
""",
        ...
        "device_list_template": """从自然语言中提取设备配置列表，我的需求如下：
...
""",
        ...
        "device_detail_template": """根据设备"{device}"生成详细的功能块配置。
...
"""
    ...
    # 数据类型约束
    "data_types": {
        "allowed_types": ["int", "float", "bool", "string", "time"]
    },
